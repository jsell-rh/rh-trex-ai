...
**Last Run By:** Codex (reconcile skill — raw resource view implementation)
...
| GAP-083 | CG-006 | Shared Resource Table Page | closed | minor | The catalog and descriptor collections share `ResourceTableComponent`; active ascending/descending markers are protected left prefixes that remain visible under right-side ellipsis truncation. |
...
| GAP-084 | CG-006 | Shared Detail and Stream Pages | closed | minor | `DetailStreamComponent` now also presents the selected decoded API object as indented sanitized JSON in a `Raw <resource>` viewport and returns to list or detail without changing selection or table state. |
...
| GAP-091 | CG-006 | Presentation Component Conformance Gate | closed | major | Unit and `teatest` coverage proves selected-row raw JSON structure and sanitization, contextual shortcut visibility, no-request inspection, and restoration of filter, sort, selection, and navigation state. |
