...
type Generator struct {
	outputDir   string
	rhtexAIPath string // local filesystem path to rh-trex-ai checkout (for go.mod replace)
}

// NewGenerator creates a Generator that writes into outputDir.
// rhtexAIPath is the absolute path to the local rh-trex-ai checkout used in the
...
// All trusted components live in github.com/openshift-online/rh-trex-ai, so there
...
{{ if .RhtexAIPath }}
replace github.com/openshift-online/rh-trex-ai => {{ .RhtexAIPath }}
{{ end }}`))

type goModData struct {
	AppName     string
	AppModule   string
	RhtexAIPath string // absolute path to rh-trex-ai local checkout for replace directive
}
